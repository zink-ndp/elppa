 ...
 */
class Qcformbuilder_Forms_Field_Calculation {
	...
	public static function checkbox_mode( $field, $form ){
		 ...
		 *
		 */
		return (bool) apply_filters( 'qcformbuilder_forms_checkbox_calculate_sum', true, $field, $form );
	...
	public static function get_value( $field, $form ){
		$entry_value = Qcformbuilder_Forms::get_field_data( $field[ 'ID' ], $form );
					...
					if( true == self::checkbox_mode( $field, $form ) ){
						if( ! empty(  $entry_value ) ){
							foreach ( $entry_value as $opt => $value ){
								 $entry_value[ $opt ] = Qcformbuilder_Forms_Field_Util::get_option_calculation_value( $opt, $field, $form );
							}
	...
	protected static function find_calc_value( $entry_value, $field, $form ){
		foreach ( $field[ 'config' ][ 'option' ] as $opt_id => $option ){
			if( $entry_value == $option[ 'value' ] ){
				$entry_value = Qcformbuilder_Forms_Field_Util::get_option_calculation_value( $option, $field, $form );
				break;
			}
