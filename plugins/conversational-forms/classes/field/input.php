 ...
 */
class Qcformbuilder_Forms_Field_Input extends Qcformbuilder_Forms_Field_HTML{
	...
	public static function html( array $field, array $field_structure, array $form, $value = null ){
		...
		$field_base_id = Qcformbuilder_Forms_Field_Util::get_base_id( $field, null, $form );
		if ( null === $value ) {
			if( ! empty( $field_structure [ 'field_value' ] ) ){
				$value = Qcformbuilder_Forms::do_magic_tags( $field_structure [ 'field_value' ], null, $form );
			}else{
		...
		if( in_array( $type, self::sync_fields() ) ){
			$syncer = Qcformbuilder_Forms_Sync_Factory::get_object( $form, $field, $field_base_id );
		...
		$field_classes = Qcformbuilder_Forms_Field_Util::prepare_field_classes( $field, $form );
		$mask = self::get_mask_string( $field );
		if( ! empty( $mask ) ){
			Qcformbuilder_Forms_Render_Assets::enqueue_script( 'inputmask' );
		}
		...
		if( 'number' === $type ){
			...
			$attrs[ 'type' ] = 'tel';
			$attrs[ 'class' ][] = 'wfb-credit-card ';
			$attr[ 'data-parsley-creditcard' ] = Qcformbuilder_Forms_Field_Util::credit_card_types( $field, $form );
	...
	protected static function get_mask_string( array  $field ){
		$mask = '';
		if ( 'phone' != Qcformbuilder_Forms_Field_Util::get_type( $field ) ) {
