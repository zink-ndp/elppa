 ...
 */
class Qcformbuilder_Forms_Entry_UI {
	 ...
	 * @since 1.4.0
	 *
	 * @var \Qcformbuilder_Forms_Entry_UI
	 */
	 ...
	 * @since 1.4.0
	 *
	 * @return \Qcformbuilder_Forms_Entry_UI
	 */
	...
	public function view_entry(){
		if( isset( $_POST, $_POST[ 'nonce' ], $_POST[ 'entry'], $_POST[ 'form' ] ) ){
			...
			$form = Qcformbuilder_Forms_Forms::get_form( strip_tags( $_POST[ 'form' ] ) );
			if( ! current_user_can( Qcformbuilder_Forms::get_manage_cap( 'entry-view', $form )  ) || ! wp_verify_nonce( $_POST[ 'nonce' ], 'wfb_view_entry' ) ){
			...
			if( 0 < $entry_id && is_array( $form ) ){
				$entry = Qcformbuilder_Forms::get_entry( $entry_id, $form );
	...
	public function view_entries(){
		...
		if( ! current_user_can( Qcformbuilder_Forms::get_manage_cap( 'entry-view', $form ) ) || ! wp_verify_nonce( $_POST['nonce' ], 'view_entries' ) ){
			wp_send_json_error( $_POST );
		}
		...
		$data = Qcformbuilder_Forms_Admin::get_entries( $form, $page, $perpage, $status );
	 ...
	 * @since 1.5.0
	 *
	 * @uses "qcformbuilder_forms_entry_actions"
	 */
	public static function get_entry_actions(){
		 ...
		 */
		$viewer_buttons_array = apply_filters( 'qcformbuilder_forms_entry_viewer_buttons', array());
		$viewer_buttons = null;
		 ...
		 * @params array $buttons
		 */
		$editor_buttons_array = apply_filters( 'qcformbuilder_forms_entry_editor_buttons', array());
		$editor_buttons = null;
		...
		if( current_user_can( 'edit_others_posts' ) ){
			echo '{{#if ../../is_active}}<button class="hidden button button-small cfajax-trigger edit-entry-btn" id="edit-entry-{{_entry_id}}" data-active-class="current-edit" data-static="true" data-load-class="spinner" ' . $editor_buttons . ' data-modal-element="div" data-group="editentry" data-entry="{{_entry_id}}" data-form="{{../../form}}" data-request="' . esc_url( Qcformbuilder_Forms::get_submit_url() ) . '{{../../form}}/{{_entry_id}}/?wfb-api={{../../form}}" data-method="get" data-modal="view_entry" data-modal-width="700" data-modal-height="auto" data-modal-title="' . esc_attr__( 'Editing Entry ', 'qcformbuilder-forms' ) . ' #{{_entry_id}}" type="button" >' . esc_html__( 'Edit', 'qcformbuilder-forms' ) . '</button> {{/if}}';



		}
		echo '{{#if ../../is_active}}<button class="button button-small ajax-trigger view-entry-btn" id="view-entry-{{_entry_id}}" data-active-class="current-view"  data-static="true" data-load-class="spinner" ' . $viewer_buttons . ' data-group="viewentry" data-entry="{{_entry_id}}" data-form="{{../../form}}" data-action="get_entry" data-modal="view_entry" data-modal-width="700" data-modal-height="700" data-modal-title="' . esc_attr__('Entry', 'qcformbuilder-forms' ) . ' #{{_entry_id}}" data-template="#view-entry-tmpl" type="button" data-nonce="' .  wp_create_nonce( 'wfb_view_entry'  ) . '">' . esc_html__( 'View', 'qcformbuilder-forms' ) . '</button> {{/if}}';
		if( current_user_can( 'delete_others_posts' ) ){
			echo '<button type="button" class="button button-small ajax-trigger" data-load-class="active" data-panel="{{#if ../../is_trash}}trash{{/if}}{{#if ../../is_active}}active{{/if}}" data-do="{{#if ../../is_trash}}active{{/if}}{{#if ../../is_active}}trash{{/if}}" data-callback="wfb_refresh_view" data-form="{{../../form}}" data-active-class="disabled" data-group="row{{_entry_id}}" data-load-element="#entry_row_{{_entry_id}}" data-action="wfb_bulk_action" data-items="{{_entry_id}}">{{#if ../../is_trash}}' . __('Restore', 'qcformbuilder-forms' ) . '{{/if}}{{#if ../../is_active}}' . esc_html_x( 'Trash', 'Verb: Action of moving to trash', 'qcformbuilder-forms' ) . '{{/if}}</button>';
		}
		/*
		if ( current_user_can( Qcformbuilder_Forms::get_manage_cap( 'resend-email' ) ) ) {
			echo '<button href="' . esc_url( add_query_arg( '_wfb_resend', Qcformbuilder_Forms_Admin_Resend::resend_nonce(), admin_url() ) ) . '&e={{_entry_id}}&f={{../../form}}" class=" button button-small  edit-entry-btn" data-active-class="current-edit" title="' . esc_attr( __( 'Click to resend email from this message', 'qcformbuilder-forms' ) ) . ' ">' . esc_html__( 'Resend', 'qcformbuilder-forms' ) . '</button>';
		}
		*/
	 ...
	 * @since 1.5.0
	 *
	 * @uses "qcformbuilder_forms_manage_cap"
	 *
	...
	public static function is_public( array $form ){
		return apply_filters( 'qcformbuilder_forms_entry_viewer_public', false, $form );
