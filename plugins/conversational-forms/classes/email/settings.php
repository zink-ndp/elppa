...
/**
 * Handles settings for Qcformbuilder Forms emails
 *
 ...
 */
class Qcformbuilder_Forms_Email_Settings {
	 ...
	 * @var string
	 */
	protected static $option_key = '_qcformbuilder_forms_email_api_settings';
	 ...
	 * @var string
	 */
	protected static $nonce_action = 'wfb-emails';
	 ...
	 * Save email settings
	 * 
	 * @uses "wp_ajax_wfb_email_save" action
	 * 
	 * @since 1.3.5
	 */
	...
	public static function maybe_add_hooks() {
		...
		if( 'wp' !== self::get_method() ){
			foreach ( self::allowed_apis() as $api ) {
				if ( self::valid( $api ) ) {
					add_filter( 'qcformbuilder_forms_mailer', array( 'Qcformbuilder_Forms_Email_Callbacks', $api ), 26, 3 );
					break;
				}
	...
	protected static function allowed_apis() {
		 ...
		 */
		return apply_filters( 'qcformbuilder_forms_allowed_email_apis', array(
			'wp',
			'sendgrid',
			'qcformbuilder'
		) );
	 ...
	 *
	 * @uses "qcformbuilder_forms_admin_footer" action
	 *
	 * @since 1.4.0
	 */
	public static function ui() {
		if (  ! version_compare( PHP_VERSION, '5.4.0', '>=' ) ) {
            echo'<div id="wfb-email-settings-ui" aria-hidden="true" style="visibility: hidden;">' . esc_html__( 'Switching email services requires PHP 5.4 or later. PHP 5.6 is strongly recommended.', 'qcformbuilder-forms' ) .'</div>';
		}else{
			include WFBCORE_PATH . '/ui/emails/settings.php';
	 ...
	 * @since 1.4.0
	 *
	 * @uses "pre_update_option__qcformbuilder_forms_email_api_settings"
	 *
