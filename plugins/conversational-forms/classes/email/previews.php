 ...
 */
class Qcformbuilder_Forms_Email_Previews {
	...
	/**
	 * Qcformbuilder_Forms_Email_Previews constructor.
	 *
	...
	public function __construct( $id = null, $view = false ) {
		if( $view && null != $id ){
			...
			add_filter( 'qcformbuilder_forms_mailer', array( $this, 'maybe_create_preview' ), 99, 3 );
		}
	...
	public function view(){
		if ( isset( $this->id ) ) {
			...
			if ( is_object( $preview ) && ! empty( $preview ) ) {
				...
				include WFBCORE_PATH . 'ui/emails/email-preview.php';
				exit;
			}else{
				wp_die( esc_html__( 'There is no saved email preview. Please submit this form with email previewing on and then try again.', 'qcformbuilder-forms' ) );
			}
	...
	public function maybe_create_preview( $mail, $data, $form ){
		if( ! empty( $form[ 'mailer' ][ 'preview_email' ] ) ){
			$this->id = $form[ 'ID' ];
			$preview = new Qcformbuilder_Forms_Email_Preview( $mail  );
			$this->record( $preview );
		}
	 ...
	 * @since 1.4.0
	 * 
	 * @param Qcformbuilder_Forms_Email_Preview $preview Preview object
	 */
	protected function record( Qcformbuilder_Forms_Email_Preview $preview ){
	...
	protected function key( ){
		return '__wfb_email_preview_' . $this->id;
