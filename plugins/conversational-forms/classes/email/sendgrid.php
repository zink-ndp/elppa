<?php
/**
 * API Client for sending Qcformbuilder Forms emails via SendGrid
 *
...
class Qcformbuilder_Forms_Email_SendGrid extends Qcformbuilder_Forms_Email_Client{
	...
	public function set_api( array $keys ){
		...
		$options = apply_filters( 'qcformbuilder_forms_sendgrid_options', $options  );
	...
	public function send(){
		...
		if( ! empty( $this->attachments ) ){
			/* @var Qcformbuilder_Forms_Email_Attachment $wfb_attachment_obj */
			foreach ( $this->attachments as $wfb_attachment_obj ) {
				$attachment = new Attachment();
				$attachment->setContent( $wfb_attachment_obj->get_encoded() );
				$attachment->setType( $wfb_attachment_obj->type );
				$attachment->setFilename( $wfb_attachment_obj->filename );
				$attachment->setDisposition( 'attachment' );
				$attachment->setContentId( md5( $wfb_attachment_obj->filename ) );
		 ...
		 */
		$mail = apply_filters( 'qcformbuilder_forms_sendgrid_before', $mail, $this->message );

		//@TODO Qcformbuilder Exception
		try {
