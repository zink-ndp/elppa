 ...
 */
class Qcformbuilder_Forms_API_Entries extends Qcformbuilder_Forms_API_CRUD {
	 ...
	 * Get an entry
	 *
	 * GET /wfb-api/v2/entries/form-id/entry-id
	 *
	 * @since 1.5.0
	 *
	...
	public function get_item( WP_REST_Request $request ) {
		...
		$entry = new Qcformbuilder_Forms_Entry( $this->form->toArray(), $request[ 'entry_id' ] );

		if( null == $entry->get_entry() ){
			return Qcformbuilder_Forms_API_Response_Factory::error_entry_not_found();
		}

		$data = $this->add_entry_to_response( $entry, array() );
		$data = $data[ $request[ 'entry_id' ] ];
		return Qcformbuilder_Forms_API_Response_Factory::entry_data( $data, 1, 1 );
	 ...
	 * Get entries
	 *
	 * GET /wfb-api/v2/entries/form-id
	 *
	 * @since 1.5.0
	 *
	 * @param WP_REST_Request $request
	...
	public function get_items( WP_REST_Request $request ) {
		...
		$entries = new Qcformbuilder_Forms_Entry_Entries( $this->form->toArray(), $per_page );
		...
		return Qcformbuilder_Forms_API_Response_Factory::entry_data(
			$data,
			count( $data ),
			$pages
		);
	 ...
	 * Delete an entry
	 *
	 * DELETE /wfb-api/v2/entries/form-id/entry-id
	 *
	 * @since 1.5.0
	 *
	 * @param WP_REST_Request $request
	...
	public function delete_item( WP_REST_Request $request ) {
		...
		$result = Qcformbuilder_Forms_Entry_Bulk::delete_entries( array( $request[ 'entry_id' ] ) );
		if( $result ){
			return new Qcformbuilder_Forms_API_Response( __( 'Entry Deleted', 'qcformbuilder-forms' ), 200, array() );
		}else{
			return new Qcformbuilder_Forms_API_Error( 'not-deleted', __( 'Entry Not Deleted', 'qcformbuilder-forms' ) );
		}
	}
     ...
     *
     * GET /wfb-api/v2/entries/form-id/delete
     *
     * @since 1.7.0
     *
     * @param WP_REST_Request $request
    ...
    public function delete_entries( WP_REST_Request $request ) {
        ...
        if( false === Qcformbuilder_Forms_Forms::is_internal_form( $formID ) ){
            $data = array(
                'deleted' => false,
                'message' =>  __( 'Form not found', 'qcformbuilder-forms')
            );
            return new Qcformbuilder_Forms_API_Response( $data, 404, array() );
        }

        $entries = \qcformbuilderwp\QcformbuilderFormsQueries\QcformbuilderFormsQueries()->selectByFormId(  $formID, false );
        ...
        if( null != $entries ) {
            ...
            Qcformbuilder_Forms_Entry_Bulk::delete_entries( $entryIds );
            $data = array(
                'deleted' => true,
                'message' =>  __( 'Entries deleted', 'qcformbuilder-forms')
            );
            ...
            $data = array(
                'deleted' => false,
                'message' =>  __( 'No entries found', 'qcformbuilder-forms')
            );
            return new Qcformbuilder_Forms_API_Response( $data, 404, array() );
        }
	 ...
	 * @since 1.5.0
	 *
	 * @param array $entries Array of found Qcformbuilder_Forms_Entry objects
	 *
	 * @return array
	 */
	protected function prepare_entries_for_response( $entries ){
		...
		if ( ! empty( $entries ) ) {
			/** @var Qcformbuilder_Forms_Entry $entry Entry Object */
	 ...
	 * @since 1.5.0
	 *
	 * @param Qcformbuilder_Forms_Entry $entry Entry object
	 * @param array $response_data Current response collection
	 *
	 * @return array
	 */
	protected function add_entry_to_response( Qcformbuilder_Forms_Entry $entry,  array $response_data ){
		...
		if( ! empty( $metas ) ){
			/** @var Qcformbuilder_Forms_Entry_Meta $meta */
		 ...
		 * @param Qcformbuilder_Forms_Entry $entry Entry data
		 */
		$response_data[ $id ] = apply_filters( 'qcformbuilder_forms_api_entry_data', $response_data[ $id ], $entry );
		return $response_data;
	}
	...
	public function get_items_permissions_check( WP_REST_Request $request ){
		$form_id =  $request[ 'form_id' ];
		$allowed = current_user_can( Qcformbuilder_Forms::get_manage_cap( 'entry-view' ), $form_id );
		...
		/**
		 * Filter permissions for viewing entries via Qcformbuilder Forms REST API
		 *
		 * @since 1.5.0
		 *
		 ...
		 */
		return apply_filters( 'qcformbuilder_forms_api_allow_entry_view', $allowed, $form_id, $request );
	...
	public function create_item_permissions_check( WP_REST_Request $request ){
		...
		/**
		 * Filter permissions for creating, updating or deleting entries via Qcformbuilder Forms REST API
		 *
		 * @since 1.5.0
		 *
		 * @param bool $allowed Is request authorized?
		 ...
		 * @param WP_REST_Request $request The current request
		 */
		return apply_filters( 'qcformbuilder_forms_api_allow_entry_edit', $allowed, $request[ 'form_id' ], $request );
