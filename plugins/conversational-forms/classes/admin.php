...
class Qcformbuilder_Forms_Admin {
			...
			<# if ( data.html ) { #>
				...
				<# } else { #>
					<div class="wpview-error">
						<div class="dashicons dashicons-wfb-logo"></div><p style="font-size: 13px;"><?php _e( 'Invalid Form.', 'qcformbuilder-forms' ); ?></p>
					</div>
					<# } #>
		</script>
		<?php
		...
		<div class="wfb-templates-wrapper">
			...
			foreach( $form_templates as $template_slug => $template ){
				if( !empty( $template['template'] ) && !empty( $template['name'] ) ){
					...
					echo '<label class="qcformbuilder-grid wfb-form-template' . $selected_template . '">';
						echo '<small>' . $template['name'] . '</small>';

						echo '<input type="radio" name="template" value="' . $template_slug . '" class="wfb-template-select"' . $selected_field . '>';
									...
									foreach( $fields as $field ){
										if( isset( $template['template']['fields'][ $field ] ) ){
											if( $template['template']['fields'][ $field ]['type'] == 'button'){
												echo '<small class="wfb-preview-field wfb-preview-button">' . $template['template']['fields'][ $field ]['label'] .'</small>';
												...
												echo '<small class="wfb-preview-field" style="height:50px;">' . $template['template']['fields'][ $field ]['label'] .'</small>';
											}elseif( $template['template']['fields'][ $field ]['type'] == 'hidden'){
												// nope- nothing
											}else{
												echo '<small class="wfb-preview-field">' . $template['template']['fields'][ $field ]['label'] .'</small>';
											}
										}
									}
									echo '</div>';
	...
	public function get_form_preview(){
		...
		if( !empty( $form ) ){
			ob_start();
			wp_head();
			echo Qcformbuilder_Forms::render_form( $form );
	...
	public function shortcode_insert_button(){
		...
		if(!empty($post)){
			...
			echo "	<img src=\"". WFBCORE_URL . "assets/images/qcformbuilder-globe-logo-sm.png\" alt=\"". esc_attr__( 'Insert Form Shortcode' , 'qcformbuilder-forms') . "\" style=\"padding: 0px 2px 0px 0px; width: 16px; margin: -2px 0px 0px;\" /> ".__('Qcformbuilder Form', 'qcformbuilder-forms' )."\n";
			echo "</a>\n";
		}
	}
	...
	public function register_admin_page(){
		...
		if( current_user_can( Qcformbuilder_Forms::get_manage_cap() ) ){
			...
			$this->screen_prefix[] = add_menu_page(
				__('Chatbot Form Builder', 'qcformbuilder-forms' ),
				__('Chatbot Form Builder', 'qcformbuilder-forms' ),
				...
				foreach($forms as $form_id=>$form){
					...
					if(!empty($form['pinned']) && !empty( $form['pin_roles'] ) ){
						...
						if( empty( $this->screen_prefix ) ){
							...
							$this->screen_prefix[] = add_menu_page( __('Chatbot Form Builder', 'qcformbuilder-forms' ), __('Chatbot Form Builder', 'qcformbuilder-forms' ), $capability, $main_slug, array( $this, 'render_admin' ), 'dashicons-format-aside', 52.999 );

						}

						$this->screen_prefix[] 	 = add_submenu_page( $main_slug, __('Chatbot Form Builder', 'qcformbuilder-forms' ).' - ' . $form['name'], $form['name'], $capability, $this->plugin_slug . '-pin-' . $form_id, array( $this, 'render_admin' ) );
	...
	public function render_admin(){
		...
		echo "	<div class=\"wrap\">\r\n";
		if(!empty($_GET['edit'])){
			echo "<form method=\"POST\" action=\"admin.php?page=" . $this->plugin_slug . "\" data-load-element=\"#save_indicator\" data-sender=\"ajax\" class=\"qcformbuilder-forms-options-form edit-update-trigger\">\r\n";
	...
	static function save_form(){
		...
		if(!empty($_GET['delete']) && !empty($_GET['cal_del']) && current_user_can( Qcformbuilder_Forms::get_manage_cap( 'save' ), strip_tags( $_GET[ 'delete' ] ) ) ){
			...
			if ( ! wp_verify_nonce( $_GET['cal_del'], 'wfb_del_frm' ) ) {
				...
				wp_die( __('Sorry, please try again', 'qcformbuilder-forms' ), __('Form Delete Error', 'qcformbuilder-forms' ) );
			}else{
				...
				if ( $deleted ) {
					...
					wp_die( __('Form could not be deleted.', 'qcformbuilder-forms' ) );
				}
		...
		if(!empty($_GET['export-form']) && current_user_can( Qcformbuilder_Forms::get_manage_cap( 'export', strip_tags( $_GET[ 'export-form' ] ) ) )){
			...
			if(empty($form)){
				wp_die( __('Form does not exist.', 'qcformbuilder-forms' ) );
			}
		...
		if(!empty($_GET['export']) && current_user_can( Qcformbuilder_Forms::get_manage_cap( 'export', strip_tags( $_GET[ 'export' ] ) ) ) ){
			...
			if(!empty($_GET['tid'])){
				...
				if(!empty($items)){
					...
					wp_die( __('Export selection has expired', 'qcformbuilder-forms' ) , __('Export Expired', 'qcformbuilder-forms' ) );
				}
			}
			...
			FROM `" . $wpdb->prefix ."wfb_form_entries` AS `entry`
			

			WHERE `entry`.`form_id` = %s
			" . $filter . "
			AND `entry`.`status` = 'active'
			ORDER BY `entry`.`datestamp` DESC;", Qcformbuilder_Forms_Sanitize::sanitize($_GET['export'])));
