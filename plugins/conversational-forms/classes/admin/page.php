...
class Qcformbuilder_Forms_Admin_Page
{
    ...
    /**
     * Used to form menu slug `qcformbuilder-forms-$page_postfix`
     *
     * @since 1.7.0
     *
     * @var  string
    ...
    /**
     * Qcformbuilder_Forms_Admin_Page constructor.
     *
     * @since 1.7.0
     *
     * @param string $page_postfix Used to form menu slug `qcformbuilder-forms-$page_postfix`
    ...
    {
        $label = sprintf(
            '<span class="qcformbuilder-forms-menu-dashicon"><span class="dashicons dashicons-%s"></span>%s</span>', esc_attr($this->menu_icon), $this->menu_label);
    ...
    public function render()
    {
        ...
        qcformbuilder_forms_print_wfb_forms_var($handle);
        wp_enqueue_script('wp-api-request');
        Qcformbuilder_Forms_Admin_Assets::set_wfb_admin(Qcformbuilder_Forms_Render_Assets::make_slug($handle));
         ...
         * @since 1.7.0
         *
         * @param Qcformbuilder_Forms_Admin_Page $page
         */
        do_action('qcformbuilder_forms_client_element_rendered', $this);
    ...
    public function enqueue_assets()
    {
        ...
        if (!empty($this->assets['scripts'])) {
            foreach ($this->assets['scripts'] as $handle) {
                Qcformbuilder_Forms_Render_Assets::enqueue_script($handle);
            }
        }

        if (!empty($this->assets['styles'])) {
            foreach ($this->assets['styles'] as $handle) {
                Qcformbuilder_Forms_Render_Assets::enqueue_style($handle);
            }
        }

        Qcformbuilder_Forms_Admin_Assets::enqueue_style('editor-grid');
    ...
    {
        return Qcformbuilder_Forms::PLUGIN_SLUG . '-';
    }
