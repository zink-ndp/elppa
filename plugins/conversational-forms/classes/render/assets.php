 ...
 */
class Qcformbuilder_Forms_Render_Assets
{
	...
	{
		if (0 === strpos($script, 'wfb-')) {
			$slug = $script;
		} elseif (wp_script_is('wfb-' . $script, 'registered')) {
			$slug = 'wfb-' . $script;
		} elseif (wp_script_is('wfb-' . $script . '-scripts', 'registered')) {
			$slug = 'wfb-' . $script . '-scripts';
		} elseif (wp_style_is('wfb-' . $script, 'registered')) {
			$slug = 'wfb-' . $script;
		} elseif (wp_style_is('wfb-' . $script . '-styles', 'registered')) {
			$slug = 'wfb-' . $script . '-styles';
		} else {
			$slug = 'wfb-' . sanitize_key(basename($script));
		}
	...
	{
		if (0 === strpos($style, 'wfb-')) {
			$slug = $style;
		} elseif (wp_style_is('wfb-' . $style . '-styles', 'registered')) {
			$slug = 'wfb-' . $style . '-styles';
		} elseif (wp_style_is('wfb-' . $style, 'registered')) {
			$slug = 'wfb-' . $style;
		} else {
			$slug = 'wfb-' . sanitize_key(basename($style));
		}
	...
	public static function get_style_includes()
	{
		 ...
		 * @since unknown
		 *
		 * @param array $style_includes To include or not. Default is value of option "_qcformbuilder_forms_styleincludes"
		 */
		$style_includes = apply_filters('qcformbuilder_forms_get_style_includes', $style_includes);
	...
	public static function get_core_styles()
	{
		$style_urls = [
			...
			'grid' => self::make_url('qcformbuilder-grid', false),
			'form' => self::make_url('qcformbuilder-form', false),
			'alert' => self::make_url('qcformbuilder-alert', false),
			'field' => self::make_url('fields', false),
			'front' => self::make_url('qcformbuilder-forms-front', false),
			'font' => self::make_url('cfont', false),
			'table' => self::make_url('qcformbuilder-table', false),
	...
	public static function get_core_scripts()
	{
		$script_urls = [
			...
			'handlebars' => WFBCORE_URL . 'assets/js/handlebars.js',
			...
			'form-front' => self::make_url('qcformbuilder-forms-front'),
	...
	public static function register()
	{
		...
		if (self::should_minify(true)) {
			...
			wp_register_script($slug, self::make_url('vue'), ['jquery'], WFBCORE_VER, true);
		}
		...
		/**
		 * Filter script URLS for Qcformbuilder Forms on the frontend, before they are enqueued.
		 *
		 * @since 1.3.1
		 *
		 * @param array $script_urls array containing all urls to register
		 */
		$script_style_urls[ 'script' ] = apply_filters('qcformbuilder_forms_script_urls', $script_urls);

		/**
		 * Filter style URLS for Qcformbuilder Forms on the frontend, before they are enqueued.
		 *
		 * @since 1.3.1
		 *
		 * @param array $script_urls array containing all urls to register
		 */
		$script_style_urls[ 'style' ] = apply_filters('qcformbuilder_forms_style_urls', $style_urls);
		...
		foreach ($script_style_urls[ 'style' ] as $style_key => $style_url) {
			...
			wp_register_style($slug, $style_url, [], WFBCORE_VER);
		...
		foreach ($script_style_urls[ 'script' ] as $script_key => $script_url) {
			...
			if ('validator' == $script_key) {
				wp_register_script(self::make_slug($script_key), $script_url, ['jquery'], WFBCORE_VER, false);
				continue;
		...
		foreach (self::get_field_styles() as $style) {
			...
			self::$registered[ 'styles' ][] = $slug;
			wp_register_style($slug, $style, [], WFBCORE_VER);
		 ...
		 */
		do_action('qcformbuilder_forms_assets_registered', $script_style_urls);


	}

	/**
	 * Enqueue a style for Qcformbuilder Forms front-end
	 *
	 * @since 1.5.0
	 *
	...
	public static function enqueue_style($style)
	{
		...
		if (!wp_style_is($style, 'enqueued')) {
			if (false !== strpos($style, '//')) {
				...
				wp_enqueue_style($slug, $style, [], WFBCORE_VER);
	...
	/**
	 * Enqueue a script for Qcformbuilder Forms front-end
	 *
	 * @since 1.5.0
	 *
	...
	public static function enqueue_script($script, $depts = ['jquery'])
	{
		...
		if ('editor-grid' === $script) {
			return Qcformbuilder_Forms_Admin_Assets::enqueue_script($script);
		}


		if (in_array($script, ['validator', self::make_slug('validator')])) {
			$scripts = self::get_core_scripts();
			wp_enqueue_script(self::make_slug('validator'), $scripts[ 'validator' ], [], WFBCORE_VER, false);
		...
		if (!wp_script_is($slug, 'enqueued') && wp_script_is($slug, 'registered')) {
			...
			if (!empty($depts) && is_array($depts) && !filter_var($script, FILTER_VALIDATE_URL)) {
				...
				wp_enqueue_script($slug, $script, $depts, WFBCORE_VER, false);
			}
		...
		if (Qcformbuilder_Forms::settings()->get_cdn()->combine()) {
			Qcformbuilder_Forms_CDN_Init::get_cdn()->add_to_combiner($slug);
		}
	...
	/**
	 * Create a URL for a script/style used by Qcformbuilder Forms
	 *
	 * @since 1.5.0
	 *
	...
	public static function make_url($name, $script = true)
	{
		...
		//@SEE https://github.com/QcformbuilderWP/Qcformbuilder-Forms/issues/2487#issuecomment-388445315
		if ('edit' === $name) {
			$min = false;
		}
	...
	public static function should_minify($script = true)
	{
		 ...
		 */
		return apply_filters('qcformbuilder_forms_render_assets_minify', true, $script);
	}
	 ...
	 * @since 1.8.4
	 */
	public static function wfb_dependencies($tag){
	...
	public static function enqueue_form_assets()
	{
		...
		self::enqueue_script('render', self::wfb_dependencies('render') );
		...
		if (!is_array(self::$api_data_localized) || !isset(self::$api_data_localized[ $field_script_to_localize ])) {
			wp_localize_script($field_script_to_localize, 'CF_API_DATA', [
				'rest' => [
					'root' => esc_url_raw(Qcformbuilder_Forms_API_Util::url()),
					'rootV3' => esc_url_raw(Qcformbuilder_Forms_API_Util::url('', false, 'v3')),
					'fileUpload' => esc_url_raw(Qcformbuilder_Forms_API_Util::url('file', false, 'v3')),
					'tokens' => [
						'nonce' => esc_url_raw(Qcformbuilder_Forms_API_Util::url('tokens/form')),
					],
					'nonce' => wp_create_nonce('wp_rest'),
				],
				'strings' => [
					'cf2FileField' => [
						'removeFile' => esc_attr__('Remove file', 'qcformbuilder-forms'),
						'defaultButtonText' => esc_attr__('Drop files or click to select files to Upload',
							'qcformbuilder-forms'),
						'fileUploadError1' => esc_attr__('Error: ', 'qcformbuilder-forms'),
						'fileUploadError2' => esc_attr__(' could not be processed', 'qcformbuilder-forms'),
						'invalidFiles' => esc_attr__('These Files have been rejected : ', 'qcformbuilder-forms'),
						'checkMessage' => esc_attr__('Please check files type and size', 'qcformbuilder-forms'),
						'invalidFileResponse' => esc_attr__('Unknown File Process Error', 'qcformbuilder-forms'),
						'fieldIsRequired' => esc_attr__('Field is required', 'qcformbuilder-forms'),
						'filesUnit' => esc_attr__('bytes', 'qcformbuilder-forms'),
						'maxSizeAlert' => esc_attr__('This file is too large. Maxium size is ', 'qcformbuilder-forms'),
						'wrongTypeAlert' => esc_attr__('This file type is not allowed. Allowed types are ',
							'qcformbuilder-forms'),
					],
				],
				'nonce' => [
					'field' => Qcformbuilder_Forms_Render_Nonce::nonce_field_name(),
				],
			]);
	...
	{
		if (!did_action('qcformbuilder_forms_assets_registered')) {
			self::register();
		}
	}
	...
	protected static function register_script($script_key, $script_url, $depts = ['jquery'])
	{
		...
		if ('field' == $script_key) {
			...
			add_filter('qcformbuilder_forms_render_assets_minify', '__return_false', 51);
			$script_url = self::make_url(str_replace('-', '/', $script_key));
			self::$registered[ 'scripts' ][] = $slug;
			wp_register_script($slug, $script_url, $depts, WFBCORE_VER, true);
			remove_filter('qcformbuilder_forms_render_assets_minify', '__return_false', 51);
			return;
		} elseif (in_array($script_key, [
			'state-events',
			'state-state',
	...
	{
		$fields = Qcformbuilder_Forms_Fields::get_all();
		$scripts = [];
	...
	public static function maybe_validator_i18n($register = true)
	{
		 ...
		 */
		$print_strings = apply_filters('qcformbuilder_forms_print_translation_strings_in_footer', false, $locale);
		...
		if ($register) {
			...
			if ($print_strings) {
				wp_localize_script(self::make_slug('validator'), 'CF_VALIDATOR_STRINGS', [
					'defaultMessage' => __( "This value seems to be invalid.", 'qcformbuilder-forms' ),
					'type' => [
						'email' => __("This value should be a valid email.", 'qcformbuilder-forms' ),
						'url' => __("This value should be a valid url.", 'qcformbuilder-forms' ),
						'number' => __("This value should be a valid number.", 'qcformbuilder-forms' ),
						'integer' => __("This value should be a valid integer.", 'qcformbuilder-forms' ),
						'digits' => __("This value should be digits.", 'qcformbuilder-forms' ),
						'alphanum' => __("This value should be alphanumeric.", 'qcformbuilder-forms' ),
					],
					'notblank' => __("This value should not be blank.", 'qcformbuilder-forms' ),
					'required' => __("This value is required", 'qcformbuilder-forms' ),
					'pattern' => __("This value seems to be invalid.", 'qcformbuilder-forms' ),
					'min' =>__( "This value should be greater than or equal to %s.", 'qcformbuilder-forms' ),
					'max' => __("This value should be lower than or equal to %s.", 'qcformbuilder-forms' ),
					'range' =>__( "This value should be between %s and %s.", 'qcformbuilder-forms' ),
					'minlength' => __("This value is too short. It should have %s characters or more.", 'qcformbuilder-forms' ),
					'maxlength' => __("This value is too long. It should have %s characters or fewer.", 'qcformbuilder-forms' ),
					'length' => __("This value length is invalid. It should be between %s and %s characters long.", 'qcformbuilder-forms' ),
					'mincheck' => __("You must select at least %s choices.", 'qcformbuilder-forms' ),
					'maxcheck' => __("You must select %s choices or fewer.", 'qcformbuilder-forms' ),
					'check' => __("You must select between %s and %s choices.", 'qcformbuilder-forms' ),
					'equalto' => __("This value should be the same.", 'qcformbuilder-forms' ),
				]);
			...
			Qcformbuilder_Forms_Render_Util::add_inline_data($script, ['ID' => rand()]);
	...
	protected static function get_validator_locale_url($locale)
	{
		...
		$validator_url = WFBCORE_URL . 'assets/js/i18n/' . $locale . '.js';
	...
	{
		if (file_exists(WFBCORE_PATH . 'assets/js/i18n/' . $locale . '.js')) {
			// no need to check other possibilities- break if/else early

		} elseif (file_exists(WFBCORE_PATH . 'assets/js/i18n/' . strtolower($locale) . '.js')) {
			$locale = strtolower($locale);
		} elseif (file_exists(WFBCORE_PATH . 'assets/js/i18n/' . strtolower(str_replace('_', '-', $locale)) . '.js')) {
			$locale = strtolower(str_replace('_', '-', $locale));
		} elseif (file_exists(WFBCORE_PATH . 'assets/js/i18n/' . strtolower(substr($locale, 0, 2)) . '.js')) {
			$locale = strtolower(substr($locale, 0, 2));
		} elseif (file_exists(WFBCORE_PATH . 'assets/js/i18n/' . strtolower(substr($locale, 3)) . '.js')) {
