...
if( $sync ){
	...
	echo '<script type="text/html" id="'. esc_attr( $syncer->template_id() ) . '">';
		echo do_shortcode( Qcformbuilder_Forms::do_magic_tags( wpautop(  $syncer->get_default() ) ) );
	echo '</script>';
	...
	$content = do_shortcode( Qcformbuilder_Forms::do_magic_tags( wpautop( $html_template ) ) );
