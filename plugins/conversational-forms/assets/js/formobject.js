...
$.fn.formJSON = function(){
  ...
  /* new button handler */
  $('.qcformbuilder_forms_form').on('click','.wfb-form-trigger', function( ev ){
    var clicked = $(this);
        form = clicked.closest('form.qcformbuilder_forms_form'),
        form_id = form.prop('id'),
        qcformbuilderforms = window[ form_id ],
        data = form.formJSON(),
        fields = {},
        target = clicked.data('target');

        for( var field in qcformbuilderforms ){
          fields[ qcformbuilderforms[ field ].slug ] = data[ field ];
        }
  ...
  if( cfModals ){
    ...
    for( var style in cfModals.style ){
      if( ! $('#wfb-' + style + '-styles-css').length ){
        head.append('<link id="wfb-' + style + '-styles-css" rel="stylesheet" type="text/css" href="' + cfModals.style[ style ] + '">');
      }
    }
    ...
    function wfb_modals_load_form( form_modal ){

      if( !form_modal.data('form') ){return;}

      var url = '/wfb-api/' + form_modal.data('form') + '/';
      if( form_modal.data('entry') ){
        url += form_modal.data('entry') + '/';
      } 
      // set form instance count
      url += '?wfb_instance=' + ( $('.qcformbuilder_forms_form.' + form_modal.data('form') ).length + 1 );
    ...
    $('.qcformbuilder-forms-modal').each( function(){
      var form_modal = $(this),
          ...
          $('body').append('<div class="remodal" data-form="' + form_id + '" ' + entry + ' data-remodal-id="modal-' + form_id + '" id="modal-' + form_id + '"><button data-remodal-action="close" class="remodal-close"></button><div class="modal-content" id="modal-' + form_id + '-content"><span class="qcformbuilder-grid wfb_processing wfb_modal"></span></div></div>');
          wfb_modals_load_form( $('#modal-' + form_id) );
    ...
    $(document).on('cf.submission', function (e, obj ) {      
      setTimeout( function(){
        ...
        wfb_modals_load_form( inst.$modal );
