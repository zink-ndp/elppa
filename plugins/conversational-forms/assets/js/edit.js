...
function pre_compile_templates(){
    var pretemplates = jQuery('.wfb-editor-template');
...
jQuery(document).ready(function($){
    ...
    $( '#qcformbuilder-forms-restore-revision' ).on( 'click', function(e){
        e.preventDefault();
        var $el = $(this);
        $spinner.addClass('loading');

        $.post({
            url: 'admin.php?page=qcformbuilder-forms',
            data:{
                wfb_edit_nonce: $( '#wfb_edit_nonce' ).val(),
                form: $el.data( 'form' ),
                wfb_revision: $( '#form_db_id_field' ).val(),
                restore: true
            },
            success: function(){
    ...
    $('.qcformbuilder-header-save-button').baldrick({
        method			:	'POST',
        request			:	'admin.php?page=qcformbuilder-forms',
        before			: function(el, e){
            ...
            $(el).data('wfb_edit_nonce', data_fields.wfb_edit_nonce);
     ...
     *	configs are stored in the .qcformbuilder-config-field-setup field within the parent wrapper
     *
     */
    function build_fieldtype_config(el){
        ...
        var select 			= $(el);
        var val = select.val();
        var parent			= select.closest('.qcformbuilder-editor-field-config-wrapper'),
            target			= parent.find('.qcformbuilder-config-field-setup'),
            template 		= get_compiled_template( val ),
            config			= parent.find('.field_config_string').val(),
            current_type	= select.data('type');


        parent.find('.qcformbuilder-config-group').show();
        ...
        if( !$('.qcformbuilder-select-field-type').not('.field-initialized').length){
            if(!core_form){
                core_form = jQuery('.qcformbuilder-forms-options-form');
            }

            // build previews
            if(!core_form.hasClass('builder-loaded')){

                var fields = $('.qcformbuilder-select-field-type.field-initialized');
            ...
            $('.qcformbuilder-header-save-button').prop('disabled', false);
    ...
    function build_field_preview(id){
        ...
        var panel           = $('#' + id);
        var select			= panel.find('.qcformbuilder-select-field-type');
        var val             = select.val();
        var
    ...
    function switch_active_group(id){
        var fields_panel	= $('.qcformbuilder-editor-fields-panel'),
            groups_panel	= $('.qcformbuilder-editor-groups-panel'),
            group_navs		= $('.qcformbuilder-group-nav'),
            group_line		= $('[data-group="'+ id +'"]'),
            group_name		= group_line.find('.qcformbuilder-config-group-name'),
            group_slug		= group_line.find('.qcformbuilder-config-group-slug'),
            group_desc		= group_line.find('.qcformbuilder-config-group-desc'),
            group_admin		= group_line.find('.qcformbuilder-config-group-admin'),
            ...
            field_lists		= $('.qcformbuilder-editor-fields-list ul'),
            group_repeat	= group_line.find('.qcformbuilder-config-group-repeat'),
            repeat_button	= $('.repeat-config-button'),
            group_settings	= $('.qcformbuilder-editor-group-settings'),
            deleter 		= $('.qcformbuilder-config-group-remove'),
            group_field_lists;
        ...
        group_field_lists = $('.qcformbuilder-editor-fields-list ul.active li');
        ...
        $('.qcformbuilder-editor-field-config-wrapper').hide();
    ...
    $('body').on('click', '.toggle_processor_event input', function(e){
        var clicked = $(this),
            ...
            settings = parent.find('.qcformbuilder-config-processor-setup'),
            notice = parent.find('.qcformbuilder-config-processor-notice');
    ...
    $('body').on('click', '.toggle_option_tab > a', function(e){
        ...
        var clicked = $(this),
            ...
            panels = clicked.closest('.qcformbuilder-editor-config-wrapper').find('.wrapper-instance-pane');
        ...
        $('.qcformbuilder-conditionals-usetype').trigger('change');
    });


    // Change Field Type
    $('.qcformbuilder-editor-body').on('change', '.qcformbuilder-select-field-type', function(e){
        // push element to config function
        build_fieldtype_config(this);
    });

    // build group navigation
    $('.qcformbuilder-editor-body').on('click', '.qcformbuilder-group-nav a', function(e){
    ...
    $('.qcformbuilder-editor-body').on('click', '.qcformbuilder-editor-fields-list a', function(e){
        ...
        // remove active field
        $('.qcformbuilder-editor-fields-list li.active').removeClass('active');
    ...
    $('.qcformbuilder-editor-body').on('click', '.group-config-button', function(e){
        var clicked = $(this),
            group_settings	= $('.qcformbuilder-editor-group-settings'),
            parent = clicked.closest('.qcformbuilder-editor-fields-panel'),
            deleter = $('.qcformbuilder-config-group-remove');

        // check if children
        if(parent.find('.qcformbuilder-field-line').length){
            // has fields
            deleter.hide();
        }else{
            deleter.show();
    ...
    $('.qcformbuilder-editor-body').on('keydown', '.field-config', function(e){
        if($(this).is('textarea')){
            return;
        }
        if(e.which === 13){
    ...
    // field label bind
    $('.qcformbuilder-editor-body').on('change', '.field-label', function(e){
        var field 		= $(this).closest('.qcformbuilder-editor-field-config-wrapper').prop('id');
        var field_line	= $('[data-field="' + field + '"]'),
            field_title	= $('#' + field + ' .qcformbuilder-editor-field-title, .layout-form-field.field-edit-open .layout_field_name'),
    ...
    // rename group
    $('.qcformbuilder-editor-body').on('change', '.active-group-name', function(e){
        e.preventDefault();
        var active_group		= $('.qcformbuilder-group-nav.active'),
            group				= active_group.data('group'),
            group_name			= active_group.find('.qcformbuilder-config-group-name'),
    ...
    // rename group slug
    $('.qcformbuilder-editor-body').on('change', '.active-group-slug', function(e){
        e.preventDefault();

        var active_group		= $('.qcformbuilder-group-nav.active'),
            group				= active_group.data('group'),
            group_name			= active_group.find('.qcformbuilder-config-group-name').val(),
            group_slug			= active_group.find('.qcformbuilder-config-group-slug'),
            group_label			= active_group.find('span'),
    ...
    $('.qcformbuilder-editor-body').on('change', '.active-group-desc', function(e){
        e.preventDefault();

        var active_group		= $('.qcformbuilder-group-nav.active'),
            group				= active_group.data('group'),
            group_desc			= active_group.find('.qcformbuilder-config-group-desc');
    ...
    // set group admin
    $('.qcformbuilder-editor-body').on('change', '.active-group-admin', function(e){
        e.preventDefault();

        var active_group		= $('.qcformbuilder-group-nav.active'),
            group				= active_group.data('group'),
            group_name			= active_group.find('.qcformbuilder-config-group-name').val(),
            group_admin			= active_group.find('.qcformbuilder-config-group-admin'),
            group_label			= active_group.find('span'),
            slug_sanitized		= this.value.split(' ').join('_').split('-').join('_').replace(/[^a-z0-9_]/gi, '').toLowerCase();
    ...
    // set repeatable
    $('.qcformbuilder-editor-body').on('click', '.repeat-config-button', function(e){
        e.preventDefault();
        var active_group		= $('.qcformbuilder-group-nav.active'),
            group				= active_group.data('group'),
            icon				= active_group.find('a .group-type'),
            group_repeat		= active_group.find('.qcformbuilder-config-group-repeat'),
            clicked				= $(this);
    ...
    // bind delete field
    $('.qcformbuilder-editor-body').on('click', '.delete-field', function(){
        var clicked = $(this),
            field	= clicked.closest('.qcformbuilder-editor-field-config-wrapper').prop('id');
    ...
    $('.qcformbuilder-editor-body').on('click', '.add-new-group,.add-field', function(){
        ...
        if( clicked.hasClass( 'add-field' ) ){
            ...
            field_input.appendTo( $('.qcformbuilder-editor-fields-list ul.active') ).focus();
        }else{
            var group_input = $('<input type="text" class="new-group-input block-input">');
            group_input.appendTo( $('.qcformbuilder-editor-groups-panel') ).focus();
        }

    });

    // dynamic group creation
    $('.qcformbuilder-editor-body').on('change keypress', '.new-group-input', function(e){
        ...
        var group_name 	= this.value,
            input		= $(this),
            wrap		= $('.qcformbuilder-editor-groups-panel ul'),
            field_list	= $('.qcformbuilder-editor-fields-list'),
            new_templ,
            new_group;
        ...
        if( !group_name.length ){
            ...
            new_templ = Handlebars.compile( $('#qcformbuilder_group_line_templ').html() );
            new_group = {
    ...
    $('.qcformbuilder-editor-body').on('change keypress', '.new-field-input', function(e){
        ...
        var new_name 	= this.value,
            input		= $(this),
            wrap		= input.parent(),
            field_conf	= $('.qcformbuilder-editor-field-config'),
            new_templ,
            new_conf_templ,
            new_field,
            deleter = $('.qcformbuilder-config-group-remove');
        ...
        if( !new_name.length ){
            ...
            deleter.hide();
            // field line template
            new_templ = Handlebars.compile( $('#qcformbuilder_field_line_templ').html() );
            // field conf template
            new_conf_templ = Handlebars.compile( $('#qcformbuilder_field_config_wrapper_templ').html() );
            ...
            new_field = {
                ...
                "group"	:	$('.qcformbuilder-group-nav.active').data('group')
            };
    ...
    $('.qcformbuilder-editor-body').on('change', '.field-slug', function(e){
        if(this.value.length){
            this.value = this.value.split(' ').join('_').split('-').join('_').replace(/[^a-z0-9_]/gi, '').toLowerCase();
        }else{
            if(e.type === 'change'){
                this.value = $(this).closest('.qcformbuilder-editor-field-config-wrapper').find('.field-label').val().split(' ').join('_').split('-').join('_').replace(/[^a-z0-9_]/gi, '').toLowerCase();
            }
        }
    });

    // bind add group button
    $('.qcformbuilder-editor-body').on('click', '.qcformbuilder-add-group', function(e){
    ...
    $('.qcformbuilder-editor-body').on('click', '.qcformbuilder-config-group-remove', function(e){

        var group = $('.active-group-slug').val();

        $('[data-group="' + group + '"]').hide(0, function(){
            $(this).remove();
            var navs = $('.qcformbuilder-group-nav');

            if(navs.length){
                navs.first().find('a').trigger('click');
            }else{
                $('.qcformbuilder-editor-fields-panel').hide();
            }
        });
    ...
    $('.qcformbuilder-editor-body').on('change record', '.field-config', function(e){

        var field 	= $(this),
            parent 	= field.closest('.qcformbuilder-editor-field-config-wrapper');
    ...
    $('.qcformbuilder-editor-body').on('focus', '.qcformbuilder-field-bind', function(e){
        var field = $(this),
            value = this.value;
    ...
    $('.qcformbuilder-editor-body').on('change', '.qcformbuilder-conditional-field-set', function(e){
        ...
        var field = $(this),
            ...
            curval = target.find('.qcformbuilder-conditional-value-field').first(),
            field_wrapper,
            is_button,
            options_wrap = [];
        ...
        if( field_id.substr(0,1) !== '{' ){
            ...
            options_wrap = field_wrapper.find('.qcformbuilder-config-group-toggle-options');
        }
        ...
        if(options_wrap.length){
            var options_rows = options_wrap.find('.toggle_option_row'),
                out = '<select name="' + name + '[value]" class="qcformbuilder-processor-value-bind qcformbuilder-conditional-value-field" data-field="' + field_id + '" style="max-width: 220px; width: 220px;">';
            ...
            out = '<input name="' + name + '[value]" type="text" class="qcformbuilder-conditional-value-field magic-tag-enabled" value="' + (target.data('value') ? target.data('value') : '') + '" style="max-width: 205px;">';
        }
    ...
    $('.qcformbuilder-editor-body').on('change', '.qcformbuilder-conditionals-usetype', function(e){
    ...
    // conditionals
    $('.qcformbuilder-editor-body').on('click', '.remove-conditional-line', function(e){
        e.preventDefault();
        var clicked = $(this),
            line = clicked.closest('.qcformbuilder-condition-line'),
            group = clicked.closest('.qcformbuilder-condition-group');

        line.remove();
        if(!group.find('.qcformbuilder-condition-line').length){
            group.remove();
        }
    ...
    // load fist  group
    $('.qcformbuilder-group-nav').first().find('a').trigger('click');

    // toggle set values
    $('.qcformbuilder-editor-body').on('change', '.toggle_show_values', function(e){
    	var $clicked = $(this),
			$wrap =  $clicked.closest('.qcformbuilder-config-group-toggle-options'),
    ...
    // autopopulate
    $('.qcformbuilder-editor-body').on('change', '.auto-populate-type', function(){
        $(this).closest('.wrapper-instance-pane').find('.auto-populate-options').trigger('change');
    });
    $('.qcformbuilder-editor-body').on('change', '.auto-populate-options', function(){
        var clicked 	= $(this),
            wrap		= clicked.closest('.wrapper-instance-pane'),
            manual		= wrap.find('.qcformbuilder-config-group-toggle-options'),
            autotype_wrap 	= wrap.find('.qcformbuilder-config-group-auto-options'),
        ...
        autotype_wrap.find('.qcformbuilder-config-group-auto-' + autotype.val()).show();
    ...
    $('body').on('keyup  focus select click init.magic', '.magic-tag-enabled', function(e){
        ...
        var input = $(this),
            wrap = input.parent(),
            fieldtype = ( wrap.closest('.wrapper-instance-pane').find('.qcformbuilder-select-field-type').val() ? wrap.closest('.wrapper-instance-pane').find('.qcformbuilder-select-field-type').val() : 'hidden' ),
                ...
                for( var tp in system_values ){
                    ...
                    if(tp !== 'system' && tp !== 'variable' && tp !== 'field'){
                        var type_instance_confs = jQuery(".processor-" + tp),
                            wrapper = input.closest('.qcformbuilder-editor-processor-config-wrapper'),
    ...
    $('.qcformbuilder-select-field-type').not('.field-initialized').each(function(k,v){
...
init_magic_tags = function(){
    ...
    magicfields.each(function(k,v){
        ...
        if(input.hasClass('magic-tag-init-bound')){
            ...
            if(input.hasClass('qcformbuilder-conditional-value-field')){
                wrapper.width('auto');
            }
...
rebuild_field_binding = function(){
    ...
    var fields = jQuery('.qcformbuilder-editor-field-config-wrapper');//.not('.bound_field');
...
rebind_field_bindings = function(){
    ...
    //return;
    var bindings = jQuery('.qcformbuilder-field-bind').not('.bound_field'),
        type_instances,
        processor_li;
    ...
    for(var v = 0; v < bindings.length; v++){
        ...
        var field = jQuery(bindings[v]),
            ...
            count = 0,
            wrapper = field.closest('.qcformbuilder-editor-processor-config-wrapper'),
            wrapper_id = wrapper.prop('id'),
            valid = '';
                    ...
                    if( system_values[type].tags && system_values[type].tags[types[t]]){
                        ...
                        for( var instance = 0; instance < type_instances.length; instance++){
                            ...
                            if(jQuery('li.'+type_instances[instance]).index() > jQuery('li.'+wrapper_id).index() && type_instances[instance] !== '__system__'){
                                if( field.closest('.qcformbuilder-editor-processors-panel-wrap').length ){
    ...
    jQuery('.qcformbuilder-header-save-button').prop("disabled", false);
    if( undefined != typeof  wfb_revisions_ui ){
        wfb_revisions_ui();
    }
};
...
function check_required_bindings(el){
    ...
    var fields,
        savebutton = jQuery('.qcformbuilder-header-save-button'),
        field_elements = jQuery('.layout-form-field'),
        nav_elements = jQuery('.qcformbuilder-processor-nav'),
        all_clear = true;

    if(el){
        fields = jQuery(el);
    }else{
        fields = jQuery('.qcformbuilder-config-field .required');
    }
    ...
    fields.each(function(k,v){
        var field = jQuery(v),
            panel = field.closest('.qcformbuilder-config-editor-panel');
        ...
        if(!v.value.length){
            ...
            var is_field = field.closest('.qcformbuilder-editor-field-config-wrapper'),
                is_process = field.closest('.qcformbuilder-editor-processor-config-wrapper');
            ...
            if( field.hasClass('toggle_value_field') ){
                var vals = field.closest('.qcformbuilder-config-group').find('.toggle_value_field').not(field);
    ...
    for(var t in required_errors){
        jQuery('.qcformbuilder-forms-options-form').find('a[href="#' + t + '"]').append('<span class="error-tag">' + required_errors[t] + '</span>');
    }

    jQuery('.qcformbuilder-conditional-field-set').trigger('change');
...
// bind pages tab
    ...
    $( ".qcformbuilder-config-editor-main-panel" ).on('click', '.qcformbuilder-add-row', function(e){
        e.preventDefault();
        var wrap = $('.page-active');
        if(!wrap.length){
            $('.qcformbuilder-add-page').trigger('click');
            return;
        }
    ...
    // edit row
    $('.qcformbuilder-editor-header').on('click', '.column-sort .icon-edit', function(e){
    ...
    $('.qcformbuilder-editor-header').on('click', '.qcformbuilder-editor-header-nav a', function(e){
        ...
        // hide all tabs
        $('.qcformbuilder-editor-body').hide();
    ...
    $('body').on('submit', '.qcformbuilder-forms-options-form', function(e){
    ...
    //toggle_option_row
    $('.qcformbuilder-editor-body').on('click', '.add-toggle-option', function(e){
        ...
        var	wrapper		= clicked.closest('.qcformbuilder-editor-field-config-wrapper'),
    ...
    // presets
    $('.qcformbuilder-editor-body').on('change', '.preset_options', function(e){
        var select = $( this ),
            preset = select.val(),
    ...
    $('.qcformbuilder-editor-body').on('click', '.toggle-remove-option', function(e){
        var triggerfield = $(this).closest('.qcformbuilder-editor-field-config-wrapper').find('.field-config').first();
        $(this).parent().remove();
        triggerfield.trigger('change');
        $(document).trigger('option.remove');
    });

    $('.qcformbuilder-editor-body').on('click', '.page-toggle', function(e){
        var clicked = $(this),
            wrap = clicked.parent(),
    ...
    $('.qcformbuilder-editor-body').on('blur toggle.values', '.toggle_label_field', function(e){
    ...
    wfb_clear_puler = function(){
        if( is_pulsating ){
            clearTimeout( is_pulsating );
            $(document).off('mouseover', '.layout-new-form-field, .column-fieldinsert', wfb_clear_puler);
        }
        $('.layout-new-form-field, .column-fieldinsert').fadeIn();
    };
    $(document).on('mouseover', '.layout-new-form-field, .column-fieldinsert', wfb_clear_puler );
    // build fild bindings
    rebuild_field_binding();
    ...
    function build_processor_sortables(){
        // set sortable groups
        $( ".qcformbuilder-editor-processors-panel ul" ).sortable({
            update: function(){
                rebuild_field_binding();
            },
            /**
             ...
             */
            create: function() {
                if( 0 == $( '.qcformbuilder-editor-processors-panel ul' ).children().length) {
                    $newProcessorButton.addClass('button-primary');
                    addProcessorButtonPulser = new QcformbuilderFormsButtonPulse( $newProcessorButton );
    ...
    $('body').on('click', '.qcformbuilder-processor-nav a', function(e){

        e.preventDefault();

        var clicked = $(this);

        $('.qcformbuilder-processor-nav').removeClass('active');
        $('.qcformbuilder-editor-processor-config-wrapper').hide();
        $( clicked.attr('href') ).show();
        clicked.parent().addClass('active');
    ...
    $('body').on('click', '.add-new-processor', function(e){
        ...
        wrap = $('.active-processors-list'),
            process_conf = $('.qcformbuilder-editor-processor-config'),
        ...
        $('.qcformbuilder-processor-nav a').last().trigger('click');
    ...
    $('body').on('click', '.delete-processor', function(e){
        ...
        var clicked = $(this),
            parent = clicked.closest('.qcformbuilder-editor-processor-config-wrapper'),
        ...
        $('.qcformbuilder-processor-nav a').first().trigger('click');
    ...
    $('body').on('change', '.qcformbuilder-select-processor-type', function(e){
        var selected = $(this),
            parent = selected.closest('.qcformbuilder-editor-processor-config-wrapper'),
            title = selected.find('option[value="'+selected.val()+'"]').text(),
            title_line = parent.find('.qcformbuilder-editor-processor-title'),
            activeline = $('.qcformbuilder-processor-nav.active a');
    ...
    $( document ).on('click', '#wfb-shortcode-preview', function(){
        $(this).focus().select();
    } );
    $( document ).on('change', '.wfb-email-preview-toggle', function(){
        var clicked = $(this),
            preview_button = $('.qcformbuilder-header-email-preview-button');
        if( clicked.is(':checked') ){
    ...
    function build_processor_config(el){
        ...
        var select 			= $(el),
            ...
            parent			= select.closest('.qcformbuilder-editor-processor-config-wrapper'),
            target			= parent.find('.qcformbuilder-config-processor-setup'),
        ...
        $('.wp-baldrick').baldrick({
            ...
            before		: function(el){
                ...
                if( tr.data('addNode') && !tr.data('request') ){
                    tr.data('request', 'wfb_get_default_setting');
                }
            }
        });
    ...
    $('.qcformbuilder-select-processor-type').each(function(k,v){
...
Handlebars.registerHelper('_field', function(args) {
    ...
    out = '<select ' + ( args.hash.type ? 'data-type="' + args.hash.type + '"' : '' ) + default_val +' ' + exclude + ' name="' + this._name + '[' + args.hash.slug + ']' + is_array + '" id="' + this._id + '_' + args.hash.slug + '" class="block-input field-config qcformbuilder-field-bind' + required + '">';
...
function wfb_revisions_ui() {
    ...
    var $spinner = jQuery( '#qcformbuilder-forms-revisions-spinner' );
    $spinner.css({
        visibility: 'visible',
        float:'none'
    });
    jQuery.get(url, function (r) {
        if( r.hasOwnProperty( 'message' ) ){
            document.getElementById('qcformbuilder-forms-revisions').innerHTML = '<p class="notice notice-large notice-info">' + r.message + '</p>';
        }else{
            var data = {
                revisions: r
            };
            ...
            document.getElementById('qcformbuilder-forms-revisions').innerHTML = template(data);
        }
        ...
        jQuery('input[type=radio][name=qcformbuilder-forms-revision]').change(function() {
            jQuery( '#qcformbuilder-forms-revision-go' ).attr( 'href', jQuery( this ).data( 'edit' ) )
                .css({
               display: 'inline-block',
               visibility: 'visible'
