/*! GENERATED SOURCE FILE qcformbuilder-forms - v1.8.6 - 2019-07-23 *//**
 * API Client for Qcformbuilder Forms API for a single form
 *
 * @since 1.5.0
 *
 ...
 * @param tokens Either WordPress REST API authentication nonce as string, or object with index nonce and token (token is Qcformbuilder Forms Entry Token)
 * @param $ jQuery
 *
...
/**
 * A VueJS-powered entry viewer for Qcformbuilder Forms
 *
 * @since 1.5.0
 *
 * @param formId Form ID
...
function CFEntryViewer2( formId, formStore, entryStore, api, config ){
    ...
    return new Vue({
        ...
        el: '#qcformbuilder-forms-entries',
        components : {
            'single-entry' : {
        ...
        methods:{
            paginationButtons: function(){
                ...
                var $next = $el.find( '.qcformbuilder-forms-entry-viewer-next-button' ),
                    $prev = $el.find( '.qcformbuilder-forms-entry-viewer-prev-button' );
            ...
            notAllowed: function (r) {
                if ( 'object' != typeof  r && 404 != r.status ) {
                    ...
                    jQuery('#qcformbuilder-forms-entries-nav').remove();
                    jQuery(document.getElementById('qcformbuilder-forms-entries')).html('<div class="alert alert-warning">' + config.strings.not_allowed + '</div>');
                }
            }
        }
    });
...
jQuery( document ).ready( function ($) {
    if( 'object' == typeof CF_ENTRY_VIEWER_2_CONFIG ){
        ...
        $.when( api.getForm(), api.getEntries(1) ).then( function( d1, d2 ){
            ...
            var entriesId = typeof CF_ENTRY_VIEWER_2_CONFIG.targets == 'object' && typeof CF_ENTRY_VIEWER_2_CONFIG.targets.entries == 'string' ? CF_ENTRY_VIEWER_2_CONFIG.targets.entries : 'qcformbuilder-forms-entries';
            var navId = typeof CF_ENTRY_VIEWER_2_CONFIG.targets == 'object' && typeof CF_ENTRY_VIEWER_2_CONFIG.targets.nav == 'string' ? CF_ENTRY_VIEWER_2_CONFIG.targets.nav : 'qcformbuilder-forms-entries-nav';
            jQuery('#' + navId).remove();
            if ( 'object' == typeof r && 404 == r.status  ) {
