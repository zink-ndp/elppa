/*! GENERATED SOURCE FILE qcformbuilder-forms - v1.8.6 - 2019-07-23 */var resBaldrickTriggers;
...
(function($){
	...
	wfb_debounce = function(func, wait, immediate) {
		var timeout;		
		return function() {
	...
	calders_forms_check_conditions = function( inst_id ){
		...
		if( typeof qcformbuilder_conditionals === "undefined" || typeof qcformbuilder_conditionals[inst_id] === "undefined"){
			return;
		}
		...
		function resetValue( field, state ){
			...
			if( null !== state ){
                ...
                if ( undefined !== $field && $field.data( 'sync' ) ) {
                    new QcformbuilderFormsFieldSync($field, $field.data('binds'), $form, $, state);
                }
			}
		...
		for( var field in qcformbuilder_conditionals[ inst_id ] ){
			// each conditional
			var fieldwrapper = jQuery('#conditional_' + field);
			if(!fieldwrapper.length){
				continue;
			}
			var type	=	qcformbuilder_conditionals[ inst_id ][field].type,
			groups	=	qcformbuilder_conditionals[ inst_id ][field].group,
			trues	=	[];
	...
	calders_forms_init_conditions = function(){
		jQuery('.qcformbuilder_forms_form').on('change keyup', '[data-field]', wfb_debounce( function(e){
			var form 			= $(this).closest('.qcformbuilder_forms_form').prop('id');
			calders_forms_check_conditions( form );
		}, 10 ) );	
	};

	if(typeof qcformbuilder_conditionals !== 'undefined'){
		calders_forms_init_conditions();
		jQuery('.qcformbuilder_forms_form').find('[data-field]').first().trigger('change');
	};
})(jQuery);
/**
 ...
 * @constructor
 */
 function Qcformbuilder_Forms_Field_Config( configs, $form, $, state ){
     var self = this;
     ...
     var $submits = $form.find(':submit, .wfb-page-btn-next' );
    ...
    /**
     ...
     function handleValidationMarkup( valid, $field, message, extraClass ){
         ...
         if( ! valid ){
             $parent.addClass( 'has-error' ).append( '<span id="wfb-error-'+ $field.attr('id') +'" class="help-block ' + extraClass +'">' + message  + '</span>' );
    ...
    function fieldIsOnCurrentPage($field) {
        return ! $field.closest('.qcformbuilder-form-page').attr('aria-hidden');
    }
    ...
    function getFieldPage($field) {
        return $field.closest( '.qcformbuilder-form-page' ).data( 'formpage' );
    }
    ...
    var rangeSliders = {};
     ...
     this.phone_better = function( field ){
         ...
         var isValid = true;
         var reset = function(){
             var error = document.getElementById( 'wfb-error-'+ fieldId );
			 isValid = true;
             if( null != error ){
                 error.remove();
             }
       ...
       $('#' + field.form_id_attr + ' [data-page="next"], #' + field.form_id_attr + ' form.qcformbuilder_forms_form [type="submit"]').click( function(e){
      ...
      * @since 1.5.0
      *
      * @type {{validateCVC: Qcformbuilder_Forms_Field_Config.creditCardUtil.validateCVC, validateExp: Qcformbuilder_Forms_Field_Config.creditCardUtil.validateExp}}
      */
     this.creditCardUtil = {
...
var wfb_jsfields_init, wfb_presubmit;
(function($){

	// validation
	wfb_validate_form = function( form ){
		return form.parsley({
	...
	wfb_jsfields_init = function(){
		...
		if( typeof resBaldrickTriggers === 'undefined' && $('.qcformbuilder_forms_form').length ){
	...
	if( $('.qcformbuilder-form-page').length ){
		$('.qcformbuilder-form-page').on('keypress', '[data-field]:not(textarea)', function( e ){
			if( e.keyCode === 13 ){
				e.preventDefault();
			}
		});
	...
	// modals activation
	$(document).on('click', '.wfb_modal_button', function(e){
		e.preventDefault();
		var clicked = $(this);
		$(clicked.attr('href')).show();
	});
	$(document).on('click', '.qcformbuilder-front-modal-closer', function(e){
		e.preventDefault();
		var clicked = $(this);
			clicked.closest('.qcformbuilder-front-modal-container').hide();
	});
	// stuff trigger
	$(document).on('cf.add cf.enable cf.disable cf.pagenav', wfb_jsfields_init );

	// Page navigation
	$(document).on('click', '[data-page]', function(e){

		var clicked = $(this),
			page_box = clicked.closest('.qcformbuilder-form-page'),
			form 	 = clicked.closest('form.qcformbuilder_forms_form'),
			form_id = form.attr( 'id' ),
			instance = form.data('instance'),
			current_page = form.find('.qcformbuilder-form-page:visible').data('formpage'),
			page	 = page_box.data('formpage') ? page_box.data('formpage') : clicked.data('page') ,
			breadcrumb = $('.breadcrumb[data-form="qcformbuilder_form_' + instance + '"]'),
			next,
			prev,
			fields,
			run = true,
		...
		wfb_validate_form( form ).destroy();
		...
		if( clicked.data('page') !== 'prev' && page >= current_page ){
			fields =  $('#qcformbuilder_form_' + instance + ' [data-formpage="' + current_page + '"] [data-field]'  );

			var $this_field,
				valid;
			for (var f = 0; f < fields.length; f++) {
				$this_field = $(fields[f]);
				if( $this_field.hasClass( 'wfb-multi-uploader' ) || $this_field.hasClass( 'wfb-multi-uploader-list') ){
					continue;
				}
			...
			if( true === run && page > current_page ){
				for( var i = page - 1; i >= 1; i -- ){
					fields =  $('#qcformbuilder_form_' + instance + ' [data-formpage="' + i + '"] [data-field]'  );
		...
		if( false === run ){
			...
			wfb_validate_form( form ).validate();
			return false;
		}
		...
		if( clicked.data('page') === 'next'){
			...
			next = form.find('.qcformbuilder-form-page[data-formpage="'+ ( page + 1 ) +'"]');
			if(next.length){
			...
			prev = form.find('.qcformbuilder-form-page[data-formpage="'+ ( page - 1 ) +'"]');
			if(prev.length){
				page_box.hide().attr( 'aria-hidden', 'true' ).css( 'visibility', 'hidden' );
			...
			if(clicked.data('pagenav')){
				...
				$('#' + clicked.data('pagenav') + ' .qcformbuilder-form-page').hide().attr( 'aria-hidden', 'true' ).css( 'visibility', 'hidden' );
				$('#' + clicked.data('pagenav') + '	.qcformbuilder-form-page[data-formpage="'+ ( clicked.data('page') ) +'"]').show().attr( 'aria-hidden', 'false' ).css( 'visibility', 'visible' );
	...
	// init page errors
	var tab_navclick;
	$('.qcformbuilder-grid .breadcrumb').each(function(k,v){
		$(v).find('a[data-pagenav]').each(function(i,e){
			var tab		= $(e),
				form 	= tab.data('pagenav'),
				page	= $('#'+ form +' .qcformbuilder-form-page[data-formpage="' + tab.data('page') + '"]');
	...
	// validator
	$( document ).on('click', 'form.qcformbuilder_forms_form [type="submit"]', function( e ){
		var $clicked = $( this ),
			$form = $clicked.closest('.qcformbuilder_forms_form'),
			validator = wfb_validate_form( $form );
		...
		if( ! validator.validate() ){
			if( $('.qcformbuilder-form-page').length ) {
				var currentPage = $clicked.parents('.qcformbuilder-form-page').data('formpage');
				...
				validator.fields.forEach(function (field, i) {
					...
					var $pageParent = field.$element.parents('.qcformbuilder-form-page');
					if (undefined != $pageParent && $pageParent.length && field.$element.parents('.qcformbuilder-form-page').data('formpage') > currentPage) {
				...
				if( ! invalids.length ){
					if( future.length ){
						$form.append( '<input type="hidden" name="_wfb_future" value="' + future.toString() + '">' );
...
window.addEventListener("load", function(){
    ...
    //Catch if window.wp is undefined
		...
		if( 'object' === typeof CFFIELD_CONFIG ) {
			...
			$('.qcformbuilder_forms_form').each(function (i, el) {
				$el = $(el);
					...
					$form = $( document.getElementById( form_id ));
					 ...
					 if( wpUndefined ){
                         ...
                         if ( ! protocolChecked ) {
                             ...
                             protocolCheck = new QcformbuilderFormsCrossOriginWarning($el, $, CFFIELD_CONFIG[instance].error_strings);
                         ...
                         if ( ! jQueryChecked &&  CFFIELD_CONFIG[instance].error_strings.hasOwnProperty( 'jquery_old' ) ) {
                             ...
                             jQueryCheck = new QcformbuilderFormsJQueryWarning($el, $, CFFIELD_CONFIG[instance].error_strings);
                         ...
                         config_object = new Qcformbuilder_Forms_Field_Config( config, $(document.getElementById(form_id)), $, state );
                         config_object.init();
                         $( document ).trigger( 'cf.form.init',{
                             ...
                             nonce: jQuery( '#_wfb_verify_' + formId ).val()
                         });
					 }
 ...
 * @constructor
 */
function QcformbuilderFormsFieldSync( $field, binds, $form, $, state  ){
 ...
 * @constructor
 */
function QcformbuilderFormsResetNonce( formId, config, $ ){
 ...
 */
function qcformbuilder_forms_check_protocol( url ){
 ...
 * @constructor
 */
function QcformbuilderFormsCrossOriginWarning( $form, $, errorStrings ){
	...
	this.maybeWarn = function () {
		if( $form.find( '[name="cfajax"]').length ){
			...
			if( ! qcformbuilder_forms_check_protocol( url ) ){
				showNotice();
			}
 ...
 * @constructor
 */
function QcformbuilderFormsJQueryWarning( $form, $, errorStrings ){
