 ...
 */
class Qcformbuilder_Forms_Processor_Load {
	 ...
	 * @access private
	 *
	 * @var Qcformbuilder_Forms_Processor_Load
	 */
	private static $init;
	...
	private function load_processors() {
		 ...
		 *
		 * Uses "qcformbuilder_forms_get_form_processors" filter to add processors
		 *
		 * @since 1.3.5.3
		 */
		do_action( 'qcformbuilder_forms_pre_load_processors' );
		 ...
		 */
		$processors = apply_filters( 'qcformbuilder_forms_get_form_processors', array() );
		 ...
		 *
		 * Use: $processors = Qcformbuilder_Forms_Processor_Load::get_instance()->get_processors(); to get processors
		 *
		 * @since 1.3.5.3
		 */
		do_action( 'qcformbuilder_forms_post_load_processors' );
	...
	private function validate_processor( $processor ) {
		...
		if ( false != $processor[ 'wfb_ver' ] ) {
			if ( ! version_compare( WFBCORE_VER, $processor[ 'wfb_ver' ], '>=' ) ) {
				$this->add_notice(  __( sprintf( $pattern, $processor[ 'name'], 'Qcformbuilder Forms', $processor[ 'wfb_ver' ], WFBCORE_VER  ) ) );
				$fail = true;
			}
		...
		if ( false != $processor[ 'php_ver' ] ) {
			if ( ! version_compare( PHP_VERSION, $processor[ 'php_ver' ], '>=' ) ) {
				$this->add_notice(  __( sprintf( $pattern, $processor[ 'name'], 'PHP', $processor[ 'php_ver' ], WFBCORE_VER  ) ) );
				$fail = true;
			}
		...
		if ( false != $processor[ 'wp_ver' ] ) {
			...
			if ( ! version_compare( $wp_version, $processor[ 'wp_ver' ], '>=' ) ) {
				$this->add_notice( __( sprintf( $pattern, $processor[ 'name' ], 'WordPress ', $processor[ 'wp_ver' ], WFBCORE_VER  ) ) );
				$fail = true;
	 ...
	 * @since 1.3.0
	 *
	 * @return \Qcformbuilder_Forms_Processor_Load
	 */
	...
	protected function add_notice( $notice ) {
		if ( class_exists( 'Qcformbuilder_Forms_Admin' ) ) {
			Qcformbuilder_Forms_Admin::add_admin_notice( $notice );
		}
