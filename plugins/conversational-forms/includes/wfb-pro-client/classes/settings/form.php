...
use qcformbuilderwp\qcformbuilderforms\pro\json_arrayable;
use qcformbuilderwp\qcformbuilderforms\pro\settings;
...
class form extends json_arrayable
{
	...
	public function should_send_local()
	{
		...
		if ( $_send_local === true ) {
			return apply_filters('qcformbuilder_forms_pro_send_local', true, $this->form_id);
		} else {
			return apply_filters('qcformbuilder_forms_pro_send_local',
	...
	{
		if ( empty($this->form) ) {
			$this->form = \Qcformbuilder_Forms_Forms::get_form($this->form_id);
		}
		return $this->form;
	}
}
