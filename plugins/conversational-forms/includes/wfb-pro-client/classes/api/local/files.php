...
namespace qcformbuilderwp\qcformbuilderforms\pro\api\local;
...
/**
 * Class files
 * @package qcformbuilderwp\qcformbuilderforms\pro\api\local
 */
class files implements \Qcformbuilder_Forms_API_Route {
	...
	public function key_check(\WP_REST_Request $request)
	{
		...
		return qcformbuilder_forms_pro_compare_to_saved_keys($public, $token);
	}
	...
	public function get_file(\WP_REST_Request $request)
	{
		...
		if ( $file && file_exists($file) ) {
			...
			if( ! $this->verify_file($file,$hash ) ){
				return new \WP_Error( 412, 'Hash is not valid', 'qcformbuilder-forms' );
			}
		...
		return new \WP_Error('404', __('File not found', 'qcformbuilder-forms'));
	}
