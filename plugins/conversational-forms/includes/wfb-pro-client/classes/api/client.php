...
namespace qcformbuilderwp\qcformbuilderforms\pro\api;
...
class client extends api
{
	 ...
	 * @since 0.0.1
	 *
	 * @param \qcformbuilderwp\qcformbuilderforms\pro\api\message $message Message data
	 ...
	 *
	 * @return \qcformbuilderwp\qcformbuilderforms\pro\message|\WP_Error|null
	 */
			...
			if ( isset($body[ 'hash' ]) && isset($body[ 'id' ]) ) {
				try {
					...
					if ( !empty($anti_spam_args) && !empty($body[ 'spam_detected' ]) && true === rest_sanitize_boolean($body[ 'spam_detected' ]) ) {
						\Qcformbuilder_Forms_Entry_Update::update_entry_status($entry_id, 'spam');
					}
	 ...
	 * Delete a message
	 *
	 * @param \qcformbuilderwp\qcformbuilderforms\pro\api\message $message Message db object
	 *
	 * @return bool
	 */
	public function delete_message(\qcformbuilderwp\qcformbuilderforms\pro\api\message $message)
	{
	...
	{
		return qcformbuilder_forms_pro_app_url();
	}
