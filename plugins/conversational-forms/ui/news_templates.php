...
{{else}}
	<div class="alert error"><p><?php echo __('Unable to connect or no extensions available.', 'qcformbuilder-forms'); ?></p></div>
{{/if}}
{{/if}}
</script>
...
{{#if channels}}
	...
	{{else}}
			{{#if url}}
				data-request="{{url}}"
			{{else}}
				data-request="<?php echo WFBCORE_EXTEND_URL . '{{location}}/?version=' . WFBCORE_VER; ?>"
			{{/if}}
			{{#if template}}
