...
use qcformbuilderwp\qcformbuilderforms\cf2\Transients\TransientApiContract;
	...
	public function processFiles(array $files, array $hashes)
	{
		...
		foreach ( $files as $file ) {
			if ( !$this->isAllowedType($file) ) {
				throw new Exception(__('A file type is not allowed. Please try another.', 'qcformbuilder-forms'), 415);
			}

			if( $this->isFileTooLarge($file)){
				throw new Exception(__('A file is too large. Please try another.', 'qcformbuilder-forms'), 415);
			}
			...
			$isPrivate = \Qcformbuilder_Forms_Files::is_private($this->field);
			...
			if ( \Qcformbuilder_Forms_Files::should_add_to_media_library($this->field) ) {
				\Qcformbuilder_Forms_Files::add_to_media_library($upload, $this->field);
			}
