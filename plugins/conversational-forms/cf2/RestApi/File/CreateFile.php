...
use qcformbuilderwp\qcformbuilderforms\cf2\Fields\Handlers\Cf1FileUploader;
use qcformbuilderwp\qcformbuilderforms\cf2\Fields\Handlers\FileUpload;
...
class CreateFile extends File
{
    ...
    protected function getArgs()
    {
        return [
            ...
            'args' => [
                'hashes' => [
                    'description' => __('MD5 has of files to upload, should have same order as files arg', 'qcformbuilder-forms')
                ],
                'verify' => [
                    'type' => 'string',
                    'description' => __('Verification token (nonce) for form', 'qcformbuilder-forms'),
                    'required' => true,
                ],
                'formId' => [
                    'type' => 'string',
                    'description' => __('ID for form field belongs to', 'qcformbuilder-forms'),
                    'required' => true,
                ],
                'fieldId' => [
                    'type' => 'string',
                    'description' => __('ID for field files are submitted to', 'qcformbuilder-forms'),
                    'required' => true,
                ],
                'control' => [
                    'type' => 'string',
                    'description' => __('Unique control string for field', 'qcformbuilder-forms'),
                    'required' => true,
                ]
            ]
        ];
    ...
    public function permissionsCallback(\WP_REST_Request $request ){
        $form =  \Qcformbuilder_Forms_Forms::get_form( $request->get_param( 'formId' ) );
        if( FileFieldType::getCf1Identifier() !== \Qcformbuilder_Forms_Field_Util::get_type( $request->get_param( 'fieldId' ), $form ) ){
            return false;
        }
        return \Qcformbuilder_Forms_Render_Nonce::verify_nonce(
    ...
    public function createItem(\WP_REST_Request $request)
    {
        ...
        $field = \Qcformbuilder_Forms_Field_Util::get_field($fieldId,$this->getForm());
